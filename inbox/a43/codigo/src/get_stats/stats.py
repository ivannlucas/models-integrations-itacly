...
def threshold_diagnostics(
    ...
    logger.info("Umbral de decisión: aquel que maximiza el F1-score de la clase Fallo")
    logger.info("")
    logger.info("Umbral operativo seleccionado en validación (se usa en inferencia): %.3f", selected_threshold)
    logger.info("")
    logger.info("Umbral óptimo sobre test (no usado en inferencia, solo se muestra como referencia diagnóstica):")
    logger.info("  - Mejor threshold teórico sobre test: %.3f", best_threshold_test)
    logger.info("  - Mejor F1 de la clase Fallo alcanzable sobre test: %.4f", best_f1_test)
    logger.info("")
    logger.info("NOTA:")
    logger.info("  El threshold de test se calcula unicamente con fines analiticos")
    logger.info("  y no se utiliza en inferencia para evitar sobreajuste al test.")
    logger.info("=" * 30)
    ...
    ax.axvline(
        ...
        color="red",
        linestyle="--",
        label=f"Umbral seleccionado en train: {selected_threshold:.3f}",
    )
    ax.axvline(
        best_threshold_test,
        color="gray",
        linestyle=":",
        alpha=0.8,
        label=f"Mejor umbral en test: {best_threshold_test:.3f}",
    )
    ax.set_xlabel("Umbral")
    ax.set_ylabel("F1 Score de la clase Fallo")
    ax.set_title("F1 Score de la clase Fallo vs Umbral de decisión")
    ax.legend()
...
def save_model_results(
    ...
    best_epoch = int(np.argmax(_history_series(history, "monitor_score"))) + 1
    model_info = {
        "best_threshold": float(threshold_used),
        "best_epoch": best_epoch,
        "n_epochs_trained": len(_history_series(history, "train_loss")),
        "validation_metrics (best_epoch)": {
            "monitor_score": float(_history_series(history, "monitor_score")[best_epoch - 1]),
            "validation_f1": float(_history_series(history, "val_anomaly_f1")[best_epoch - 1]),
            "validation_auc": float(_history_series(history, "val_anomaly_auc")[best_epoch - 1]),
            "validation_acc": float(_history_series(history, "val_anomaly_acc")[best_epoch - 1]),
        },
        "test_metrics": {
