...
def create_sequences(
    ...
    target_column: str,
    seq_length: int = 50,
    solapamiento_beta: float = 1.0,
    id_column: Optional[str] = None,
    timestamp_column: str = "timestamp",
    normal_tokens: Optional[Sequence[str]] = None,
) -> tuple[np.ndarray, np.ndarray, list[tuple], list[tuple]]:
    ...
    Returns:
        Tupla con (X_seq [N,T,F], y_seq [N], timestamp_windows_labels, csv_windows_index).
    ...
    excluded_cols = {target_column, timestamp_column}
    if entity_col is not None:
        excluded_cols.add(entity_col)

    feature_cols = [c for c in df_ord.columns if c not in excluded_cols]
    if len(feature_cols) == 0:
        raise ValueError("No hay columnas de entrada para construir secuencias.")

    X_seq: list[np.ndarray] = []
    y_seq: list = []
    timestamp_windows_labels = []
    ...
    def _window_target_label(window_labels: np.ndarray) -> int:
        """Determina la etiqueta de la ventana según la presencia de anomalías.
        Devuelve 1 si hay al menos el 50% de anomalías en la ventana, 0 si no."""

        anomaly_mask = _anomaly_mask(window_labels, normal_tokens=normal_tokens)
        return int(anomaly_mask.mean() >= 0.5) 
    ...
    return np.asarray(X_seq), np.asarray(y_seq, dtype=np.int64), timestamp_windows_labels
...
def run_data_processing_pipeline(
        ...
        if os.path.isdir(data_path) and all(
            ...
            if config is None:
                raise FileNotFoundError(
                    "No se encontraron datos de entrada y no se proporcionó la "
                    "configuración completa para generar splits automáticamente."
                )

            logger.info("No se encontraron CSVs de entrada. Generando splits automáticamente...")
            from src.data_processing.generate_dataset import run_dataset_generation

            run_dataset_generation(config)

            splits_dir = config.get('paths', {}).get('splits', 'data/splits')
            split_df = {}
            for split_name, filename in split_filenames.items():
                split_path = os.path.join(splits_dir, filename)
                if not os.path.isfile(split_path):
                    raise FileNotFoundError(
                        f"No se encontró el split generado esperado: {split_path}"
                    )
                split_frame = pd.read_csv(split_path)
                split_frame.columns = split_frame.columns.str.lower()
                if id_column and id_column in split_frame.columns and timestamp_column in split_frame.columns:
    ...
    for split in split_names:
        X_seqs[split], y_seqs[split], _ = create_sequences(
            split_df[split],
            target_column=target_column,
            seq_length=data_cfg['sequence_length'],
            solapamiento_beta=data_cfg['solapamiento_beta'],
    ...
    for split in split_names:
        feature_cols = [
            c for c in split_df[split].columns
            if c not in [target_column, id_column, timestamp_column]
        ]
        df_stats[split] = stats_windows(
            X_seqs[split],
            feature_names=feature_cols,
            stats_creation=fuzzy_processing_cfg.get('stats_creation', ['mean', 'std']),
            ddof=0,
        )
