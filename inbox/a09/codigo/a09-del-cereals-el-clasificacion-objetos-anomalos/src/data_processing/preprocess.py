    ...
    *,
    ...
    if require_target and split_target_col in df.columns:
        ...
        if split_target_values.isna().any():
            raise ValueError(f"La columna {split_target_col} contiene valores no vÃ¡lidos.")
...
def prepare_processed_datasets(
    ...
    group_col = cfg["data"]["group_column"]
    ts_col = cfg["data"]["timestamp_column"]
    leak_cols = set(cfg["data"]["leakage_columns"])

    df = validate_and_normalize_input_frame(df_raw, cfg, require_target=True)
    df = df.sort_values([group_col, ts_col]).reset_index(drop=True)

    processed_path = Path(cfg["paths"]["processed_dataset"])
    ...
    return {
        ...
        "rows": str(len(df)),
        "samples": str(df[group_col].nunique()),
    }
