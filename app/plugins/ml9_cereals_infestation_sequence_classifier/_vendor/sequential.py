"""VENDORED from the a09 delivery — src/training/sequential.py.

Copied verbatim from inbox/a09/codigo/a09-del-cereals-el-clasificacion-objetos-anomalos/ so the
GRU/LSTM architecture, the StandardScaler application and the softmax inference path are
bit-for-bit identical to the AI team's pipeline. Do NOT refactor: any change here breaks
numerical reproducibility against the manifest golden_cases.

Deliberate deviations from the original file (only these):
  1. `train_sequence_model()` accepts an optional `init_state_dict`, so /train can fine-tune the
     served checkpoint instead of always starting from random weights. Passing None reproduces
     the original from-scratch behaviour exactly.
  2. `load_checkpoint()` passes weights_only=True to torch.load (the default from torch 2.6, which
     is what this repo pins). Verified against the delivered final_winner.pt — the checkpoint holds
     only tensors/dicts/primitives.
"""
...
def train_sequence_model(
    ...
    *,
    ...
    model = build_model(model_type, X_train.shape[-1], num_classes, params)
    if init_state_dict is not None:
        # Fine-tuning: start from the served checkpoint's weights instead of random init.
