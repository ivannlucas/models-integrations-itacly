...
class Ml30MeatTraceabilityDetectionPlugin(ModelPluginPort):
    ...
    def train(self, *, data_path: str) -> TrainResponse:  # pylint: disable=too-many-locals
        ...
        from app.plugins.ml30_meat_traceability_detection.constants import MODEL_FILENAME
        from app.plugins.ml30_meat_traceability_detection.model_loader import (
        ...
        for _ in range(80):  # ponytail: fixed 80 epochs, matches source retrain
            for xb, yb in loader:
                optimizer.zero_grad()
        ...
        logger.info("train() done — acc=%.4f f1=%.4f auc=%.4f", acc, f1, auc)
